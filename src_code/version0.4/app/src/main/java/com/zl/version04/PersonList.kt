            ...
            TopAppBar(
                ...
                actions = {
                    IconButton(onClick = {
                        ...
                        personData.add(mutableStateOf(Person("", "", "", id = Person.maxId + 1)))
//                        temp.value = Person("", "", "")
                                ...
                                if (title == "添加") {
                                    ...
                                    GlobalScope.launch(Dispatchers.Default) {
                                        coroutineScope {
                                            ...
                                            Log.i("zeta", "add: ${iterableTOString(personData)}")
                                            Log.i("zeta", "select $index ")
                                            Log.i("zeta", "add ${personData[index]}")
                                            Log.i(
                                                "zeta", "database: ${
                                                    StaticObj.personDataBase.PersonDataDao()
                                                        .getAll()
...
//                                            val temp = Person(person.value, _id = person.value.id)
                                            ...
                                            Log.i("zeta", "update: ${iterableTOString(personData)}")
                                            Log.i("zeta", "select $index ")
                                            Log.i("zeta", "update ${personData[index]}")
                                            Log.i(
                                                "zeta", "database: ${
                                                    StaticObj.personDataBase.PersonDataDao()
                                                        .getAll()
...
fun createPersonObj() : SnapshotStateList<MutableState<Person>> {
    ...
    for (i in StaticObj.initPersonDatabaseList) {
        Person.maxId = Person.maxId.coerceAtLeast(i.id)
